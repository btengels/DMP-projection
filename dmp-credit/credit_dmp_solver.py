...
class credit_dmp_solver(credit_dmp_model):
    ...
    def solve_model(self):
        ...
        self.results = sp.optimize.fsolve(self._residual, self.initial_guess, full_output=3)
        self.aE = self.results[0].reshape(len(self.N), len(self.X_grid))
    ...
    def _residual(self, aE_vec):
        ...
        q = self._get_q(self.aE, self.K_grid)
        ...
        inside_p  = (1 - self.s_C)*(1/(1 + self.r))*(Fg_p + self.K_grid*(1-q)/q)
    ...
    def _unpack_residual_inputs(self, aE_vec):
        ...
        self.aE = aE_vec[0:ix_aE].reshape(self.nn, len(self.X_grid))     
...
if __name__ == "__main__":
    ...
    err_after = solver._residual(solver.aE)
